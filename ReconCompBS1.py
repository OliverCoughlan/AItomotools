...
def plots(noisyImg, cleanImg, FDKpredImg,
	...
	uErr = ssim(cleanImg[0][0], UpredImg, data_range = UpredImg.max()-UpredImg.min())
	iErr = ssim(cleanImg[0][0], IpredImg, data_range = IpredImg.max()-IpredImg.min())
	...
	e = ax[1,1].imshow(UpredImg.T, vmin=0,vmax=2.5)
	f = ax[1,2].imshow(IpredImg.T, vmin=0,vmax=2.5)
	# set the titles of the subplots
	add_zoom_bubble(d)
	add_zoom_bubble(e)
	add_zoom_bubble(f)
...
itnet = ItNet().to(dev)
itnetSD = torch.load("/local/scratch/public/obc22/ItNetTrainCheckpts/FINALepoch199.pt")['model_state_dict']
