...
from ts_algorithms import  fdk #importing conventional fdk which is used in lpd
...
class dataProximal(nn.Module):
    ...
    def __init__(self, layers, channels): #initialise dual nn
    ...
    def forward(self, x):
        return self.block(x) #returns output of dual nn which is forward proj.
...
class RegProximal(nn.Module):
    ...
    def forward(self, x):
        return self.block(x) #returns output of primal nn - also forward proj.
...
class LPD(nn.Module):
    ...
    def __init__(
        ...
        #initial setup using parameters and geom defined below
        ...
        for i in range(self.model_parameters.n_iters):
            ...
            #adds primal and dual modules described above
        ...
        #autograd is a differentiation function in pytorch - used to minimise grad descent
        #used for back proj. to determine error
    ...
    def default_parameters(mode='ct'):
        ...
        return LPD_params
    #default params used to initialise class
    ...
    def forward(self, g):
        ...
        for i in range(self.model_parameters.n_iters):
            ...
            #runs backward dual proj to estimate error

            update = self.lambda_dual[i]*self.AT(h[:, :1]).cuda()
            #params updated with h
            f_primal = self.__primal_step(f_primal, update, primal_module)
            #runs primal forw proj. step toj estimate img

        return f_primal[:, 0:1]
        #takes sinogram, back projects to find error then forward projects
        #this reduces error step by step